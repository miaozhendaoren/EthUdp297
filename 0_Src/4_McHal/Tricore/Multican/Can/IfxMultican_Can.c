...
IfxMultican_Status IfxMultican_Can_MsgObj_readMessage(IfxMultican_Can_MsgObj *msgObj, IfxMultican_Message *msg)
{
    ...
    /* if successfull: */
    if (status == IfxMultican_Status_noError)
    {
        if (msgObj->msgObjCount > 1)
        {
...
IfxMultican_Status IfxMultican_Can_Node_init(IfxMultican_Can_Node *node, IfxMultican_Can_NodeConfig *config)
{
    ...
    {                           /* NPCR write enabled only of NCR.CCE set */
        ...
        {
            status |= (IfxMultican_Node_initTxPin(hwNode, config->txPin, config->txPinMode));
        }

        if (config->rxPin != NULL_PTR)
        {
            status |= (IfxMultican_Node_initRxPin(hwNode, config->rxPin, config->rxPinMode));
        }
    }
...
IfxMultican_Status IfxMultican_Can_initModule(IfxMultican_Can *mcan, const IfxMultican_Can_Config *config)
{
    ...
    return IfxMultican_Status_noError;
}
