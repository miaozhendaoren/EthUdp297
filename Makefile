...
build: $(PROJ_FIRST_SUBMAKE)
	...
	@$(MAKE) $(MULTI_PROC) -f $(PROJ_FIRST_SUBMAKE) $(BUILD_TARGET)

$(PROJ_FIRST_SUBMAKE): $(SUBDIR_MAKE_TARGET)
	@echo Now the make files are generated !!!
	$(PHP) "$(BINUTILS_PATH)"/../php/Scripts/IfxSwFw_GenMake_CSrc.php $(MAKEGEN_COMMAND)
	@echo ''

#The target "clean" will remove obj + executables
clean: $(PROJ_FIRST_SUBMAKE)
	@$(MAKE) $(MULTI_PROC) -f $(PROJ_FIRST_SUBMAKE) clean
...
indent:
	@$(MAKE) -i $(MULTI_PROC) -f $(PROJ_DIR)/1_ToolEnv/0_Build/0_Utilities/Indent.mk

#The target "doc" invokes the doxygen tool to generate the doxygen files
doc:
	@$(MAKE) $(MULTI_PROC) -f $(PROJ_DIR)/1_ToolEnv/0_Build/0_Utilities/Doxygen.mk
	
#The target "cleandoc" makes the output of documentation removed
cleandoc:
	@$(MAKE) $(MULTI_PROC) -f $(PROJ_DIR)/1_ToolEnv/0_Build/0_Utilities/Doxygen.mk clean
